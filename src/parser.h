	...
	public:
		...
		void visit(startNode& head){
			...
			for(node * child : head.children){
				if(child->semanticMeaning == "classdecl" || child->semanticMeaning =="funcdef"||child->semanticMeaning == "vardecl"){
			...
			for(node * child : head.children){
				if(child->semanticMeaning == "{" ){
					...
					for(node* actualChild : classBodyChildren){
						...
						else if(actualChild->semanticMeaning=="attributedecl"){
							node::symbolTableEntry ste;
							std::string type;
		...
		void visit(funcNode& head){
			for(node * child : head.children){
				if(child->semanticMeaning == "funchead" ){
					for(node * actualChild : child->children){
						if(actualChild->semanticMeaning =="funcparams"){
							for(node * actualActualChild : actualChild->children){
								if(actualActualChild->semanticMeaning =="param"){
									node::symbolTableEntry ste;
									ste.type = "parameter";
									for(node * paramChild : actualActualChild->children){
										if(paramChild->semanticMeaning =="id"){
											ste.name = paramChild->nodeValue;
										}
										else if(paramChild->semanticMeaning == "type"){
											ste.type = paramChild->nodeValue;
										}
				...
				if(child->semanticMeaning == "funcbody" ){
					for(node * actualChild : child->children){
						if(actualChild->semanticMeaning =="vardecl"){
							node::symbolTableEntry ste;
							ste.kind = "variable";
							for(node * varDeclChild: actualChild->children){
								if(varDeclChild->semanticMeaning =="id"){
									ste.name = varDeclChild->nodeValue;
								}
								else if(varDeclChild ->semanticMeaning =="type"){
									ste.type = varDeclChild->nodeType;
								}
							}
							head.stMap[child->nodeValue] = ste;
						}
